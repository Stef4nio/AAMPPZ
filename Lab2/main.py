...
def MFAToXFA(MFA):
    final_states = [output[0] for output in MFA['output_function'] if output[2] in MFA['output_letters']]
    DFA = {
        'states': list(set([transition[0] for transition in MFA['transition_function']] 
                           + [transition[2] for transition in MFA['transition_function']])),
        'letters': MFA['input_letters'],
        'transition_function': MFA['transition_function'],
        'start_states': MFA['start_states'],
...
def main():
    MFA = readFA('moore.json')
    XFA = MFAToXFA(MFA)
    regex = convertToRegex(XFA)
    print(regex)
